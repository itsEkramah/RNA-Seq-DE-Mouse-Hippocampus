#Differential Expression Analysis (R/DESeq2) BY USING BASH TERMINAL

cd /media/ekramah/DISK2/RNASEQ2/07_scripts/
cat << 'EOF' > DE_analysis.R
# DE_analysis.R
...
# It is designed to be run non-interactively via `Rscript` from the command line.
...
# This ensures R looks for files in the correct locations
...
rownames(df_annotation_samples) <- colnames(mat) # Use colnames(mat) here too
